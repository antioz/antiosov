(() => {
  ...
  async function products() {
    ...
    app.innerHTML = tabs('products') + `<p style="text-align:center"><a class="btn" href="#product/new" style="width:auto;padding:0 24px">+ Добавить товар</a></p>
      ...
      ${products.map(p => `<tr class="row" data-id="${p.id}"><td>${esc(p.title)}<br><small class="meta">${p.id}</small></td><td>${rub(p.price)}</td><td>${p.kind === 'digital' ? `цифровой · ${p.file_key ? 'архив загружен' : 'архива нет'}` : (p.variants || []).map(v => `${v.size !== '-' ? v.size + ': ' : ''}${v.stock - v.reserved}/${v.reserved}/${v.preorder_count}`).join(' · ')}</td><td>${p.active ? 'да' : 'нет'}</td></tr>`).join('')}</table>`;
    app.querySelectorAll('tr.row').forEach(r => r.onclick = () => location.hash = '#product/' + r.dataset.id); bindLogout();
  }

  async function product(id) {
    const isNew = id === 'new';
    const p = isNew ? { id: '', title: '', description_md: '', price: '', images: [], weight_g: 300, dims_cm: { x: 30, y: 20, z: 3 }, sizes: [], preorder_allowed: false, preorder_ship_by: '', active: false, sort: 0, variants: [], kind: 'physical', file_key: '' }
    ...
    app.innerHTML = tabs('products') + `<p class="meta"><a href="#products" style="color:inherit;text-decoration:none">← товары</a></p><form id="pf">
      ...
      <div id="dig"><div class="meta" style="margin-bottom:6px">Архив (zip)</div><p id="arch" style="font-size:14px;margin:0 0 8px"></p>
        <input type="file" id="zip" accept=".zip,application/zip" ${isNew ? 'disabled title="сначала сохраните товар"' : ''}><p class="meta" id="zupl"></p></div>
    ...
    // Архив: admin/file → presigned PUT (application/zip) → ключ в file_key и сразу сохранить товар
    app.querySelector('#zip').onchange = async e => {
      const file = e.target.files[0]; if (!file) return; const st = app.querySelector('#zupl');
      ...
      try {
        ...
        p.file_key = key; renderKind(); st.textContent = 'загружено, сохраняю…';
